...
## D-030 — The MCP can create a complete ZenID workspace from conversation

- Date: 2026-09-03
- Status: Accepted
- Decision owner: Creator
- Context: The first MCP release could safely edit an existing `.zenid` file,
  but a student who had only notes, project descriptions, or a folder of
  experience material still had to open the website and construct the first
  project manually. The creator wants the connected agent to guide that person
  from an empty workspace through a saved ZenID project and exported PDF.
- Decision: Add `zenid_create_project`, which creates the same current-schema
  local workspace in memory without writing a file, and `zenid_add_fact`, which
  appends a user-stated domain, skill, experience, project, achievement,
  certification, or education entry. New item identifiers are generated by
  ZenID, unknown fields fail loudly, and every addition is reported as a factual
  change. A newly added entry is hidden from portfolio publication until the
  user explicitly selects it in ZenID; skills use a hidden section because the
  schema has no per-skill publication list. The agent must not invent missing
  history. The first save of a newly created workspace requires an explicit
  target path.
- Consequence: A compatible agent can ask questions, build a CV from scratch,
  validate it, save a browser-compatible `.zenid` file, and export the same PDF
  as the application. Existing fact, publication, filesystem, and no-network
