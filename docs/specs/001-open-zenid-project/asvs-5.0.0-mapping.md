...
`Not applicable` means that the exact requirement has no component in this
browser-only feature scope; analogous controls are noted separately and are not
treated as evidence for that requirement. No status should be read as a
product-wide ASVS result.
...
| `v5.0.0-5.2.3` | Before decompression, bound an archive's uncompressed size and number of files. | [`projectFile.js`](../../../frontend/pdf-editor/src/resume/projectFile.js) lines 83–131 applies entry-count, per-entry, and cumulative expanded-size checks through the decompressor filter before output allocation. [`projectFile.test.js`](../../../frontend/pdf-editor/src/resume/projectFile.test.js) lines 156–205 covers expanded size, entry count, per-entry size, and inconsistent stored-entry metadata. | Evidenced | The exact pre-allocation limits are evidenced for the current `fflate` ZIP implementation, not every compression format. Near-limit responsiveness has only a single-machine measurement and still requires representative-device assessment. |
...
| `v5.0.0-5.3.3` | During server-side decompression, ignore user-provided path information to prevent ZIP-slip-style writes. | No server-side ZIP handling exists in the SPEC-001 feature scope. As an analogous client-side control, [`projectFile.js`](../../../frontend/pdf-editor/src/resume/projectFile.js) lines 34–53, 96–105, and 249–320 validates archive paths, generates export paths, rejects duplicate names, and rejects files outside the manifest-derived allowlist. Unsafe and duplicate path regressions are in [`projectFile.test.js`](../../../frontend/pdf-editor/src/resume/projectFile.test.js) lines 119–127 and 207–220. | Not applicable | ZenID decompresses in browser memory and does not write archive paths to a server filesystem. The analogous controls reduce client-side untrusted-path ambiguity but are not evidence for this explicitly server-side requirement. |
...
- authentication (V6), session management (V7), or authorization (V8), because
  the local import feature has no account, session, or access-control flow to
  assess;
- server-side transport security and TLS deployment (V12), because no server
  component participates in project import and production hosting requires a
  separate deployment assessment;
- security-log inventory, event coverage, protection, retention, or operational
  monitoring (V16), beyond the scoped import-failure row above, because browser
  error behavior is not evidence for operational logging controls;
- dependency vulnerability governance, build provenance, production
  ...
  V15 requirements), because those controls require separate inventories,
  scanning, provenance, configuration, and operational evidence.
