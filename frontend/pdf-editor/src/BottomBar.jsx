  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-30 flex -translate-x-1/2 items-center gap-3 rounded-full border border-white/10 bg-neutral-900/80 px-4 py-2 shadow-2xl shadow-black/50 backdrop-blur-xl">
      <div className="flex items-center gap-1">
        <button type="button" onClick={onPrevPage} disabled={navigationDisabled || currentPage <= 1} title="Previous page" aria-label="Previous page" className={iconButtonClass}>
          <ChevronLeft size={16} />
        </button>
        <span className="min-w-[92px] text-center text-sm text-neutral-300">
          Page {currentPage} of {numPages}
        </span>
        <button type="button" onClick={onNextPage} disabled={navigationDisabled || currentPage >= numPages} title="Next page" aria-label="Next page" className={iconButtonClass}>
