...
import { getResumeCopy } from "../localization.js";
...
export default function BuilderView({
  ...
  const interfaceResumeCopy = getResumeCopy(locale);
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try {
      await exportResumeToPdf({ template, resumeData: outputResumeData, accentColor, language: activeResume.language });
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col">
      <header className="flex flex-wrap items-center gap-3 border-b border-stone-800/50 bg-stone-900/60 px-4 py-3 backdrop-blur-sm sm:px-6">
        ...
        <select
          ...
          aria-label={t("Active resume version")}
        ...
        <input
          ...
          aria-label={t("Resume version name")}
        ...
        <label className="flex items-center gap-2 text-xs text-stone-400">
          <span className="sr-only sm:not-sr-only">{t("Output language")}</span>
          <select
            value={activeResume.language}
            onChange={(event) => onChangeResumeLanguage(event.target.value)}
            aria-label={t("Output language")}
            className="rounded-lg border border-stone-700 bg-stone-900 px-2 py-2 text-xs text-stone-200 outline-none focus:border-amber-600"
          >
        ...
        <button
          ...
          aria-label={t("Add resume variant")}
        ...
        >
          <CopyPlus size={15} /> <span className="hidden xl:inline">{t("Add Resume Variant")}</span>
        </button>
        {resumes.length > 1 && (
          <button
            type="button"
            ...
            title={t("Delete current resume variant")}
            aria-label={t("Delete current resume variant")}
          >
            <Trash2 size={15} />
          </button>
        )}
        ...
        <button
          ...
          aria-label={t("Open ZenID project")}
          className="flex items-center gap-1.5 rounded-lg border border-stone-700 px-3 py-2 text-xs font-medium text-stone-300 transition-colors hover:border-amber-600 hover:bg-amber-600/10 hover:text-amber-400"
          title="Open a private ZenID project locally"
        >
          <FolderOpen size={15} /> <span className="hidden xl:inline">{t("Open Project")}</span>
        </button>
        <button
          type="button"
          onClick={onSaveProject}
          aria-label={t("Save ZenID project")}
          className="flex items-center gap-1.5 rounded-lg border border-stone-700 px-3 py-2 text-xs font-medium text-stone-300 transition-colors hover:border-amber-600 hover:bg-amber-600/10 hover:text-amber-400"
          title="Save an editable private project to this device"
        >
          <Save size={15} /> <span className="hidden xl:inline">{t("Save Project")}</span>
        </button>
        <button
          type="button"
          onClick={onResetProject}
          className="flex items-center justify-center rounded-lg p-2 text-stone-500 transition-colors hover:bg-red-500/10 hover:text-red-400"
          title={t("Clear local project")}
          aria-label={t("Clear local project")}
        >
          <RotateCcw size={15} />
        </button>
        <a
        ...
        >
          <BookOpen size={16} />
          <span className="hidden sm:inline">{t("Resume Checklist")}</span>
        ...
        >
          <Download size={16} /> {exporting ? t("Exporting…") : t("Export PDF")}
        </button>
      </header>
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        <div className="w-full border-r border-stone-800/60 bg-stone-950 px-6 py-8 lg:w-[420px] lg:shrink-0 lg:overflow-y-auto">
          ...
          <AccordionSection title={t("Included in this resume")} defaultOpen>
            <ResumeItemSelection
          ...
          <AccordionSection title={t("Targeted wording")}>
            <TargetedWording
              resumeData={resumeData}
              selectedItems={activeResume.selectedItems}
          ...
          {sectionOrder.map((key, idx) => {
            ...
            return (
              <AccordionSection
                key={key}
                title={interfaceResumeCopy.sections[key] || SECTION_LABELS[key]}
        ...
        <div className="flex flex-1 justify-center overflow-x-auto bg-transparent px-6 py-10">
          <div className="lg:sticky lg:top-10 lg:self-start">
            <div className="mb-3 flex max-w-[595px] items-start justify-between gap-4 text-xs text-stone-400">
              <div className="flex rounded-lg border border-stone-800 bg-stone-900/70 p-0.5" aria-label={t("Preview type")}>
                <button
                  type="button"
              ...
              <span className="max-w-sm text-right">
                {previewMode === "pdf"
                  ? t("Exact ATS PDF layout and page breaks, generated locally in your browser.")
                  : template === "modern"
                    ? t("The ATS PDF uses a single-column layout. Select PDF export to inspect the exact pages.")
                    : t("Live design preview. Select PDF export to inspect exact page breaks.")}
              </span>
            </div>
            {previewMode === "pdf" ? (
              <ResumePdfPreview resumeData={outputResumeData} accentColor={accentColor} language={activeResume.language} />
            ) : (
              <ResumePreview template={template} resumeData={outputResumeData} accentColor={accentColor} language={activeResume.language} />
            )}
          </div>
        </div>
      </div>
